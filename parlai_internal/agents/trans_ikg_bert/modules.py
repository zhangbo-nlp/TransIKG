...
class TransIKGModel(TorchGeneratorModel):
    ...
    def __init__(self, opt, dictionary):
        ...
        self.embeddings = self.encoder.context_encoder.embeddings
    ...
    def output(self, tensor, encoder_states, decoder_states):
        ...
        output = F.linear(tensor, self.embeddings.word_embeddings.weight)
...
def to_bert_input(token_idx, null_idx):
    ...
    return token_idx, mask, segment_idx




class TransIKGEncoder(nn.Module):
    def __init__(self, opt, pad_idx, n_segments):
        super().__init__()
        ...
        self.knowledge_encoder = self.context_encoder

        self.embeddings = self.context_encoder.embeddings.word_embeddings
        ...
        self.use_golden_knowledge = opt.get('use_golden_knowledge', False)

    def forward(self, src_tokens, know_tokens, ck_mask, text_segments):
        # encode the context, pretty basic
        context_input, context_mask, text_segments_idx = to_bert_input(src_tokens, self.pad_idx)
        context_encoded = self.context_encoder(context_input, context_mask, text_segments_idx).last_hidden_state
        if self.use_dialogue_position:
        ...
        know_flat, know_mask, know_segment_idx = to_bert_input(know_flat, self.pad_idx)
        know_encoded = self.knowledge_encoder(know_flat, know_mask, know_segment_idx).last_hidden_state
...
class TransIKGDecoder(nn.Module):
    ...
    def forward_embedding(
        ...
        """
        tensor = self.embeddings(input, position_ids=positions)
...
class TransIKGDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.ci_ffn = TransIntegration(
            embedding_size, ffn_size, relu_dropout=relu_dropout, activation=activation, integrate_type='ci'
        )
        ...
        if self.use_overall_integration:
            self.oi_ffn = TransIntegration(
                embedding_size, ffn_size, relu_dropout=relu_dropout, activation=activation, integrate_type='oi',
            )
    ...
    def forward(  # type: ignore
        ...
        if use_dialogue_position and segments is not None:
            ...
            dpf = (torch.cos(np.pi*segments)  - 4/m * segments + 7 + (3*m+2)/(m+1)) / 8 +\
                 self.hardtanh(self.bias[segments])
            dpf = (
                dpf
                    .unsqueeze(1)
...
class TransIntegration(nn.Module):
    ...
    def __init__(self, dim, dim_hidden, relu_dropout=0., activation='relu', integrate_type=None):
