...
class TrainLoop:
    ...
    def validate(self):
        ...
        if self.valid_worlds is None:
            ...
            self.valid_worlds = load_eval_worlds(self.agent, opt, 'test')
        ...
        is_best = 2
        bleu_valid = f1_valid = rouge_valid = 0
        if not opt['skip_generation']:
            bleu_valid = valid_report['bleu-4'].value()
            f1_valid = valid_report['f1'].value()
            rouge_valid = valid_report['rouge_L'].value()
            is_best = int(bleu_valid > self.best_bleu) + int(f1_valid > self.best_f1) + int(rouge_valid > self.best_rouge)
        ...
        if (
            ...
            self.best_bleu = bleu_valid
            self.best_f1 = f1_valid
            self.best_rouge = rouge_valid
    ...
    def _run_eval(
        ...
        tasks = [world.task for world in valid_worlds]
